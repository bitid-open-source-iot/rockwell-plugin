...
class Sim extends EventEmitter {
    ...
    // connect() {
    //     try {
    //         cmd.get('sudo cell_mgmt status', (err, data, strres) => {
    //             if (err) {
    //                 console.log(err);
    //                 this.emit('disconnect', true);
    //             } else {
    ...
    async connect() {
        try {
            ...
            await cmd.run('sudo cell_mgmt stop');
            __logger.info('Restarting Sim Service');
            await cmd.run('sudo cell_mgmt start APN=internet');
            
            await cmd.run('sudo cell_mgmt status', (err, data) => {
                if (err) {
                    console.log(err);
                    this.emit('disconnect', true);
                } else {
