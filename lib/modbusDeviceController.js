    ...
    async init(){
        ...
        self.tmrPoll = setInterval(() => {
            for (let i = 0; i < self.arrOutputs.length; i++) {
                ...
                try{
                    console.log(1)
                    response = await self.device.write(self.arrOutputs[i].register, self.arrOutputs[i].value)
                    console.log(2)
                    console.log(3)
                    // console.log('myResponse', response)
                }catch(e){
                    console.error(`error writing to ${self.ip} register: ${self.arrOutputs[i].register}`)
                }
            };
